...
When a Baustelle transitions from alive to dead (or back), its petition counter rolls over rather than resetting. One KV key per site (`count:<slug>`); no separate `count:alive:<slug>` / `count:dead:<slug>`.
